...
import ThreeDRoomAudio from "../../models/v2/Audio.js";
...
router.post(
  ...
  async (req, res) => {
    ...
    const key = `stars/${starId}/three-d-rooms/${roomId}/audios/${Date.now()}-${file.originalname}`;
    await uploadToWasabi(file.path, key, file.mimetype || "audio/mpeg");
    await fs.unlink(file.path);

    const audio = await ThreeDRoomAudio.create({ roomId, title, key });
...
router.delete(
  ...
  verifyToken,
  async (req, res) => {
    const { starId, audioId } = req.params;
